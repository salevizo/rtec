...
%--------------- gap ---------------%
initiatedAt(gap(Vessel) = Status, T):-
...
happensAt(gap_init(Vessel),T):-
    happensAt(start(gap(Vessel) = nearPort), T).
happensAt(gap_init(Vessel),T):-
    happensAt(start(gap(Vessel) = farFromPorts), T).
...
holdsFor(rendezVous(Vessel1, Vessel2) = true, I) :-
    ...
    withinAreaType(Vessel1,nearCoast,Iw1),
    withinAreaType(Vessel2,nearCoast,Iw2),
